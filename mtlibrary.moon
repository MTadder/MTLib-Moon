...
class List
	...
	combine:(withTbl)=>
		if (type(withTbl) == 'table') then
			for k,v in pairs(withTbl) do @add(v, k)
	__tostring:=> _serialize(@Items, ', ')
	__len:=> #@Items
	__add:(v1, v2)->
		if (type(v1) != 'table') then return (v2\add(v1))
		elseif (type(v2) != 'table') then return (v1\add(v2))
	...
	topKey:=>
		lK = nil
		for k,v in pairs(@Items) do lK = k
		(lK)
	top:=>
...
class Dyad
	...
	equals:(o, o2)=>
		...
		elseif ((type(o2) == 'number') and (o != nil)) then
